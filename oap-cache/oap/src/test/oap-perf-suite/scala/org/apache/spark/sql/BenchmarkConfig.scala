...
trait ParquetVsOrcConfigSet extends BenchmarkConfigSelector{
  ...
  def allConfigurations: Seq[BenchmarkConfig] =
    if(conf.get("spark.sql.oap.cache.memory.manager") == "offheap" ||
      ((!conf.contains("spark.sql.oap.cache.memory.manager")) && conf.get("spark.sql.oap.fiberCache.memory.manager")=="offheap")) {
      Seq(
        new BenchmarkConfig()
          .setBenchmarkConfName("Orc w/ index")
          .setBenchmarkConf(BenchmarkConfig.FILE_FORMAT, "orc")
